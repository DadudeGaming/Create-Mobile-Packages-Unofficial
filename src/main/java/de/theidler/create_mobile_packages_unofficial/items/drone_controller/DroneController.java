...
public class DroneController extends StockCheckingItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        if (!pLevel.isClientSide) {
            ...
            if(!isTuned(stack)) {
                pPlayer.displayClientMessage(Component.translatable("item.create_mobile_packages_unofficial.drone_controller.not_linked"), true);
                return InteractionResultHolder.success(pPlayer.getItemInHand(pUsedHand));
            }
            MenuProvider provider = new SimpleMenuProvider(
                    (id, inv, p) -> new DroneControllerMenu(id, inv, this),
                    Component.translatable("item.create_mobile_packages_unofficial.drone_controller")
            );
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltip, TooltipFlag pFlag) {
        ...
        if (Screen.hasShiftDown()) {
            ...
            pTooltip.add(Component.translatable("item.create_mobile_packages_unofficial.drone_controller.tooltip.summary").withStyle(ChatFormatting.YELLOW));
            pTooltip.add(Component.literal(""));
            pTooltip.add(Component.translatable("item.create_mobile_packages_unofficial.drone_controller.tooltip.condition1").withStyle(ChatFormatting.GRAY));
            pTooltip.add(Component.translatable("item.create_mobile_packages_unofficial.drone_controller.tooltip.behaviour1").withStyle(ChatFormatting.YELLOW));
            pTooltip.add(Component.translatable("item.create_mobile_packages_unofficial.drone_controller.tooltip.condition2").withStyle(ChatFormatting.GRAY));
            pTooltip.add(Component.translatable("item.create_mobile_packages_unofficial.drone_controller.tooltip.behaviour2").withStyle(ChatFormatting.YELLOW));
        } else {
